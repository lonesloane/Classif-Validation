...
class ExcelGenerator(object):
    ...
    def fill_values(self, xls_file_path, csv_file, sheet_annotations):
        ...
        # Style of the long file name
        long_file_name_style = xlwt.easyxf(
            "font: bold true, color black;"
            "pattern: pattern solid, fore_colour gray25;")
            ...
            for row in csv_input_file:
                ...
                for cell_value in cell_values:
                    ...
                    if row_index == 0:
                        ...
                        # insert long file name if found in previous excel file...
                        if long_file_name is None:
                            print "search long file name"
                            long_file_name = self.get_file_long_name(previous_workbook,
                                                                     cell_values[0])
                            print "long file name: %s" % long_file_name
                            if long_file_name:
                                row_index += 1
                                sheet_annotations.write(row_index, 0,
                                                        long_file_name.upper(),long_file_name_style)
                            row_index += 1
    ...
    @staticmethod
    def get_file_long_name(previous_workbook, current_filename):
        long_file_name = ""
        print "current file name: %s" % current_filename
        sheet = previous_workbook.sheet_by_index(0)
        for row_index in range(sheet.nrows):
            previous_values = sheet.row_values(row_index, 0, 9)
            if (previous_values[ExcelGenerator.FILE] ==
                           current_filename):
                long_file_name = sheet.row_values(row_index-1, 0, 1)[0]
                break
